...
namespace vkz {
    ...
    public:
        explicit VertexInputStateBuilder(Device device, GraphicsPipelineBuilder *parent);

        VertexInputStateBuilder &
        addVertexBindingDescription(uint32_t binding, uint32_t stride, VkVertexInputRate inputRate);
        ...
        VertexInputStateBuilder &
        addVertexAttributeDescription(uint32_t location, uint32_t binding, VkFormat format, uint32_t offset);
        ...
        inline VertexInputStateBuilder &addVertexAttributeDescriptions(const AttributeDescriptions &attributes) {
            for (const auto &attribute: attributes) {
                addVertexAttributeDescription(attribute.location, attribute.binding, attribute.format,
                                              attribute.offset);
            }
            return *this;
        }
